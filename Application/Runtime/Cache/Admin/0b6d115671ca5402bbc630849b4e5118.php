                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">商品信息更新</h3>
                        </div><!-- /.box-header -->
                        <div class="box-body">
                            <div class="row">
                                <form action="<?php echo U('Index/good');?>" method="get">
                                    <div class="col-sm-6">
                                        <div id="example1_filter" class="dataTables_filter">
                                            <input type="hidden" name="action" value="update">
                                            <label>根据手机型号查询:<input id="searchtxt" name="GoodsName" type="search" class="form-control input-sm" placeholder="" aria-controls="example1"></label><input id="search" type="submit" class="btn btn-block btn-info" style="width:70px;" value="查询">
                                        </div>
                                    </div>
                                </form>
                            </div>
                                <table id="example2" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        <?php if(is_array($data["goods"])): $i = 0; $__LIST__ = $data["goods"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr role="row" class="odd">
                                            <form action="<?php echo U('User/update_good');?>" method="post" enctype="multipart/form-data">
                                                <td><?php echo ($vo["goods_id"]); ?></td>
                                                <td><input id="<?php echo ($vo["goods_name"]); ?>" name="goods_name" class="form-control" type="text" value="<?php echo ($vo["goods_name"]); ?>" /></td>
                                                <td><input id="<?php echo ($vo["goods_id"]); ?>" name="price" class="form-control" type="text" value="<?php echo ($vo["price"]); ?>" /></td>
                                                 <td><input name="GoodsPicture" class="form-control" type="file" /></td>
                                                <td><input name="<?php echo ($vo["goods_id"]); ?>" type="submit" class="btn btn-primary" value="修改"></td>
                                                </form>
